  ...
  async (_, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://v6.exchangerate-api.com/v6/774761ad2d6689f9ca262c67/latest/RUB"
      );
    ...
    builder
      ...
      .addCase(fetchConvertation.fulfilled, (state, action) => {
        ...
        if (state.currency && state.amount) {
          state.result = (
            action.payload[state.currency] * state.amount
          ).toFixed(2);
        }
      })
...
export const { setAmount, setCurrency, setCurrencyList } =
  convertationSlice.actions;
