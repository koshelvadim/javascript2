...
function AppWork8() {
  ...
  const {
    ...
    currencyList,
    result,
    loading: loadingConvertation,
    error: errorConvertation,
  } = useSelector((state) => state.convertation);
  ...
  return (
    <div className="App">
      <div>
        ...
        <p>
          {error
            ? error.message || "Something go wrong try again later"
            : value}
        </p>
      </div>
      ...
      <div>
        ...
        <form onSubmit={handleConvertation}>
          ...
          <button type="submit" disabled={loadingConvertation}>
            {loadingConvertation ? "loading..." : "Convartate"}
          </button>
        </form>
        <p>
          {amount} RUB = {result} {currency}
        </p>
        <p>
          {errorConvertation
            ? errorConvertation.message || "Something go wrong try again later"
            : value}
        </p>
      </div>
    </div>
